...
def draw(json_path: str):
    with open(json_path, "r", encoding="utf-8") as file:
        ...
        for file in data_json["data"]:
            ...
            plt.plot(data["recall"], qps, label=file["title"], marker="^", lw=1)
        plt.xticks(np.arange(0.9, 1.0, 0.01))
        plt.legend(loc="lower center", bbox_to_anchor=(0.5, 1), ncol=2)
        plt.xlabel("recall@10")
        plt.ylabel("QPS (1/s)")
        plt.grid(True, linestyle=":")
        plt.savefig(data_json["output_file_frontier"], dpi=300, bbox_inches="tight")
        plt.clf()
        ...
        plt.savefig(data_json["output_file_ablation"], dpi=300, bbox_inches="tight")
